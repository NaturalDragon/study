! function(e) {
    ...
    try {
            ...
            function() {
                ...
                _.map = function(e, t) {
                    ...
                    e.success = function(e) {
                        ...
                        try {
                            4 == s.readyState && (s.status >= 200 && s.status < 300 || 304 == s.status ? e.success(t(s.responseText)) : e.error(t(s.responseText), s.status), s.onreadystatechange = null, s.onload = null)
                        } catch (r) {
                    ...
                    try {
                        ...
                        }), e.data && (e.cors || s.setRequestHeader && s.setRequestHeader("X-Requested-With", "XMLHttpRequest"), "application/json" === e.contentType ? s.setRequestHeader && s.setRequestHeader("Content-type", "application/json; charset=UTF-8") : s.setRequestHeader && s.setRequestHeader("Content-type", "application/x-www-form-urlencoded"))
                    } catch (o) {
                        sd.log(o)
                    }
                    s.send(e.data || null)
                ...
                protocol: {
                    ...
                    ajax: function(e) {
                        return e !== sd.para.server_url && void(_.isString(e) && "" !== e && !this.protocolIsSame(e, location.href) && sd.log("SDK 检测到您的数据发送地址和当前页面地址的协议不一致，建议您修改成一致的协议。因为 http 页面使用 https + ajax 方式发数据，在 ie9 及以下会丢失数据。"))
                    }
                }
            };
        var commonWays = {
        ...
        sd.quick = function() {
            ...
            var t = {
                ...
                r = {
                    ...
                    notifyUser: function() {
                        ...
                        window.addEventListener && window.addEventListener("message", e, !1), r.postMessage()
                    }
                },
                s = function(e) {
                    function t() {
            ...
            t.isSeachHasKeyword() ? t.hasKeywordHandle() : window.parent !== self && r.isSearchHasKeyword() ? r.verifyVtrackMode() : t.isStorageHasKeyword() ? t.storageHasKeywordHandle() : window.parent !== self && r.isStorageHasKeyword() ? r.verifyVtrackMode() : (e(), r.notifyUser())
        }, BatchSend.prototype = {
            add: function(e) {
            ...
            send: function(e) {
                var t, r = this;
                return _.isString(sd.para.server_url) && "" !== sd.para.server_url || _.isArray(sd.para.server_url) && sd.para.server_url.length ? (t = _.isArray(sd.para.server_url) ? sd.para.server_url[0] : sd.para.server_url, void _.ajax({
                    url: t,
                    type: "POST",
                ...
                })) : void sd.log("当前 server_url 为空或不正确，只在控制台打印日志，network 中不会发数据，请配置正确的 server_url！")
            },
            ...
            readStore: function() {
                for (var e = [], t = [], r = null, s = (new Date).getTime(), a = localStorage.length, n = this.getPendingItems(), i = 0; i < a; i++) {
                    ...
                    if (0 === o.indexOf("sawebjssdk-") && /^sawebjssdk\-\d+$/.test(o)) {
                        ...
                        r = localStorage.getItem(o), r ? (r = _.safeJSONParse(r), r && _.isObject(r) ? (r._flush_time = s, e.push(o), t.push(r)) : (localStorage.removeItem(o), sd.log("localStorage-数据parse异常" + r))) : (localStorage.removeItem(o), sd.log("localStorage-数据取值异常" + r))
                    }
                }
                return {
                    keys: e,
        ...
        sd.bridge = {
            ...
            initPara: function() {
                ...
                "object" == typeof sd.para.app_js_bridge ? sd.para.app_js_bridge = _.extend({}, e, sd.para.app_js_bridge) : sd.para.use_app_track === !0 || sd.para.app_js_bridge === !0 || "only" === sd.para.use_app_track ? (sd.para.use_app_track_is_send !== !1 && "only" !== sd.para.use_app_track || (e.is_send = !1), sd.para.app_js_bridge = _.extend({}, e)) : "mui" === sd.para.use_app_track && (e.is_mui = !0, sd.para.app_js_bridge = _.extend({}, e)), sd.para.app_js_bridge.is_send === !1 && sd.log("设置了 is_send:false,如果打通失败，数据将被丢弃！")
            },
            initState: function() {
                function e(e) {
                    function t(e) {
